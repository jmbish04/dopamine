...
import { aiCostLogs, budgetEvents, sessions } from '@/db/schemas';
import { generateUuid } from '@/utils/common';
import { sql, desc, eq, gt, and } from 'drizzle-orm';
import { z } from 'zod';
import Cloudflare from 'cloudflare';
import { PRICING_CATALOG, guardCheck, type ModelPricing } from "@/backend/ai/utils/pricing-registry";
...
export class BudgetTracker {
  ...
  async checkBudgetStrict(): Promise<void> {
    const maxBudgetUsd = parseFloat(String(this.env.MAX_AI_BUDGET || '0'));
  ...
  async getCurrentSpend(): Promise<number> {
    ...
    const resetTime = lastReset?.timestamp || 0; // Default to epoch if no reset
  ...
  async getBudgetStatus() {
    const limit = parseFloat(String(this.env.MAX_AI_BUDGET || '0'));

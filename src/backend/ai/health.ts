...
import { getAIGatewayUrl } from "@/backend/ai/utils/ai-gateway";
import { cleanJsonOutput, sanitizeAndFormatResponse } from "@/backend/ai/utils/sanitizer";
import { analyzeFailure } from "@/backend/ai/utils/diagnostician";
import { HealthStepResult } from "@/health/types";
import { getGeminiApiKey, getOpenaiApiKey, getCloudflareAccountId, getAiGatewayToken } from "@utils/secrets";
...
export async function checkHealth(env: Env): Promise<HealthStepResult> {
    ...
    const aigEnvCheck: Record<string, boolean> = {
        CLOUDFLARE_ACCOUNT_ID: !!(await getCloudflareAccountId(env)),
        AI_GATEWAY_NAME: !!env.AI_GATEWAY_NAME,
        AI_GATEWAY_TOKEN: !!(await getAiGatewayToken(env)),
    ...
    if (await getCloudflareAccountId(env) && await getAiGatewayToken(env)) {
        await runCheck("aiGatewayToken", async () => {
            const accountId = await getCloudflareAccountId(env);
            if (!accountId || accountId === 'null') throw new Error("CLOUDFLARE_ACCOUNT_ID is required for token verification");

            const token = await getAiGatewayToken(env);
            const gatewayName = env.AI_GATEWAY_NAME || "dopamine";

...
export async function checkHealth(env: Env): Promise<HealthStepResult> {
    ...
    const agents = [
        { name: 'Planner', binding: env.PLANNER as any, instance: 'health-check-probe' },
        { name: 'Supervisor', binding: env.SUPERVISOR as any, instance: 'health-check-probe' }
    ];
    ...
    const overallStatus = failureCount === 0 ? 'success' : 
                          failureCount === agents.length ? 'failure' : 'skipped';
